...
ripgrep_fzf_vim() {
	...
	rg --color=always --line-number --no-heading --smart-case "${*:-}" |
		fzf --ansi \
			--color "hl:-1:underline,hl+:-1:underline:reverse,gutter:-1,selected-bg:238,selected-fg:146,current-fg:189" \
			--delimiter : \
			--preview 'bat --color=always {1} --highlight-line {2}  --theme gruvbox-dark' \
...
function imgur_upload() {
	local image_path=$1
	local client_id=$(gopass show -f -o websites/imgur.com/noahlias)
	curl -s -F "image=@${image_path}" -H "Authorization: Client-ID ${client_id}" https://api.imgur.com/3/image | jq -r '.data.link'
}

function xkcd() {
	local url="https://xkcd.com/$(shuf -i 1-2750 -n 1)/info.0.json"
	local imgurl=$(curl -s "$url" | jq ".img" | sed 's/"//g')
	local imgfile="/tmp/xkcd.png"
	# download image
	curl -s "$imgurl" -o "$imgfile"
	# resize image
	magick "$imgfile" -resize 800x800 "$imgfile"
	# show image in kitty
	kitty +kitten icat --align left "$imgfile"
}
